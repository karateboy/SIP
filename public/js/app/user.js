				...
				[
						...
						function($monitorType, $group, $user, $scope, $http) {
							...
							self.deleteUser = function(){
								$user.remove({id:$scope.selectedUser.email}, function(resp){
									if(!resp.ok){
										alert("使用者不存在!");
									}else{
										self.groupList = $group.query();
							...
							self.copyUser = function(){
								if($scope.selectedUser){
									...
									$scope.userGroup = self.groupList.find(function(g){
									...
									$scope.widgets = {};
									self.monitorTypeList.forEach(function(mt){
										if(self.user.widgets.indexOf(mt._id) != -1)
							...
							self.updateUser = function(){
								...
								user.widgets = getSelectedArrayFunction($scope.widgets)();								
								$user.update({id:user.email}, user,
